...
public class Ship
{
    public Crane.Crane Crane { get; set; }
    public Dictionary<int, Stack<Crate>> Stacks { get; private set; } = new();

    public Ship(string startingStackList, CraneVersion craneVersion = CraneVersion.V9000)
    {
        CreateStacks(startingStackList);
        Crane = CraneFactory.CreateCrane(craneVersion);
    }
